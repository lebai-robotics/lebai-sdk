...
void Gripper::GripperImpl::SetPosition(unsigned int position) {
  ...
  try {
    ...
    // Write to register at address 0x9C40 using Write Multiple Registers (FC
    // 0x10) This will generate frame: 01 10 9C 40 00 01 02 [value_bytes] [CRC]
    uint16_t values[1] = {value};
    Modbus::StatusCode status =
        client_->writeMultipleRegisters(0x9C40, 1, values);
...
unsigned int Gripper::GripperImpl::GetCurrentPosition() const {
  ...
  try {
    uint16_t values[1] = {0};
    Modbus::StatusCode status =
        client_->readHoldingRegisters(0x9C45, 1, values);
    if (!Modbus::StatusIsGood(status)) {
      std::string errorMsg =
          "Failed to read current position from gripper. Status: " +
          std::to_string(static_cast<int>(status));
      if (status == (Modbus::Status_Bad | 0x102)) {
        errorMsg += " (RTU frame error - check baud rate and device settings)";
      }
...
void Gripper::GripperImpl::SetForce(unsigned int force) {
  ...
  try {
    ...
    uint16_t values[1] = {value};
    Modbus::StatusCode status =
        client_->writeMultipleRegisters(0x9C41, 1, values);
    if (!Modbus::StatusIsGood(status)) {
    ...
    throw std::runtime_error("Failed to set force: " + std::string(e.what()));
  }
}
...
unsigned int Gripper::GripperImpl::GetCurrentForce() const {
  ...
  try {
    uint16_t values[1] = {0};
    Modbus::StatusCode status =
        client_->readHoldingRegisters(0x9C42, 1, values);
    if (!Modbus::StatusIsGood(status)) {
      std::string errorMsg =
          "Failed to read current force from gripper. Status: " +
          std::to_string(static_cast<int>(status));
      if (status == (Modbus::Status_Bad | 0x102)) {
        errorMsg += " (RTU frame error - check baud rate and device settings)";
      }
    ...
    throw std::runtime_error("Failed to get current force: " +
                             std::string(e.what()));
  }
}
...
unsigned int Gripper::GripperImpl::GetCurrentVelocity(bool persistent) const {
  ...
  try {
    ...
    if (!Modbus::StatusIsGood(status)) {
      std::string errorMsg =
          "Failed to read current velocity from gripper. Status: " +
          std::to_string(static_cast<int>(status));
      if (status == (Modbus::Status_Bad | 0x102)) {
        errorMsg += " (RTU frame error - check baud rate and device settings)";
      }
    ...
    throw std::runtime_error("Failed to get current velocity: " +
                             std::string(e.what()));
  }
}
...
void Gripper::GripperImpl::DoCalibration() {
  ...
  try {
    uint16_t values[1] = {1};
    Modbus::StatusCode status =
        client_->writeMultipleRegisters(0x9C48, 1, values);
    if (!Modbus::StatusIsGood(status)) {
      std::string errorMsg =
          "Failed to write calibration command to gripper. Status: " +
          std::to_string(static_cast<int>(status));
      if (status == (Modbus::Status_Bad | 0x102)) {
        errorMsg += " (RTU frame error - check baud rate and device settings)";
      }
    ...
    throw std::runtime_error("Failed to do calibration: " +
                             std::string(e.what()));
  }
}
...
bool Gripper::GripperImpl::IsCalibrated() const {
  ...
  try {
    uint16_t values[1] = {0};
    Modbus::StatusCode status =
        client_->readHoldingRegisters(0x9C49, 1, values);
    if (!Modbus::StatusIsGood(status)) {
      std::string errorMsg =
          "Failed to read calibration state from gripper. Status: " +
          std::to_string(static_cast<int>(status));
      if (status == (Modbus::Status_Bad | 0x102)) {
        errorMsg += " (RTU frame error - check baud rate and device settings)";
      }
    ...
    throw std::runtime_error("Failed to get calibration state: " +
                             std::string(e.what()));
  }
}

void Gripper::GripperImpl::TurnOnAutoCalibration() {
  // 40090 (0x9C9A) 关闭自动找行程指令(可写) 2 关闭自动找行程且断电保存 3
  // 恢复自动找行程且断电保存
  if (!client_ || !port_) {
    throw std::runtime_error("Modbus client is not initialized");
  }
  ...
  try {
    ...
    if (!Modbus::StatusIsGood(status)) {
      std::string errorMsg =
          "Failed to enable auto calibration setting on gripper. Status: " +
          std::to_string(static_cast<int>(status));
      if (status == (Modbus::Status_Bad | 0x102)) {
        errorMsg += " (RTU frame error - check baud rate and device settings)";
      }
    ...
    throw std::runtime_error("Failed to turn on auto calibration: " +
                             std::string(e.what()));
  }
}
  ...
  try {
    ...
    if (!Modbus::StatusIsGood(status)) {
      ...
      oss << "Failed to disable auto calibration setting on gripper. Status: 0x"
          ...
          << std::hex << std::uppercase << static_cast<int>(status);
      errorMsg = oss.str();
      if (status == (Modbus::Status_Bad | 0x102)) {
        errorMsg += " (RTU frame error - check baud rate and device settings)";
      }
