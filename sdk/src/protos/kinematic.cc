...
namespace lebai
{
  namespace kinematic
  {
    ...
    void KinData::set_actual_joint_speed(const std::vector<double> & actual_joint_speed)
    {
    ...
    void KinData::set_actual_joint_acc(const std::vector<double> & actual_joint_acc)
    {
    ...
    void KinData::set_actual_joint_torque(const std::vector<double> & actual_joint_torque)
    {
    ...
    void KinData::set_target_joint_pose(const std::vector<double> & target_joint_pose)
    {
    ...
    void KinData::set_target_joint_speed(const std::vector<double> & target_joint_speed)
    {
    ...
    void KinData::set_target_joint_acc(const std::vector<double> & target_joint_acc)
    {
    ...
    void KinData::set_target_joint_torque(const std::vector<double> & target_joint_torque)
    {
    ...
    void KinData::set_actual_tcp_pose(const posture::CartesianPose & actual_tcp_pose)
    {
    ...
    // target_tcp_pose_
    void KinData::set_target_tcp_pose(const posture::CartesianPose & target_tcp_pose)
    {
    ...
    // actual_flange_pose_
    void KinData::set_actual_flange_pose(const posture::CartesianPose & actual_flange_pose)
    {
    ...
    bool KinData::Deserialize(const rapidjson::Value &obj)
    {
      ...
      for(auto iter = obj["actual_joint_pose"].GetArray().Begin(); iter != obj["actual_joint_pose"].GetArray().End(); iter++)
      {
        actual_joint_pose_.push_back(iter->GetDouble());
      }
      actual_joint_speed_.clear();
      for(auto iter = obj["actual_joint_speed"].GetArray().Begin(); iter != obj["actual_joint_speed"].GetArray().End(); iter++)
      {
        actual_joint_speed_.push_back(iter->GetDouble());
      }
      actual_joint_acc_.clear();
      for(auto iter = obj["actual_joint_acc"].GetArray().Begin(); iter != obj["actual_joint_acc"].GetArray().End(); iter++)
      {
        actual_joint_acc_.push_back(iter->GetDouble());
      }
      actual_joint_torque_.clear();
      for(auto iter = obj["actual_joint_torque"].GetArray().Begin(); iter != obj["actual_joint_torque"].GetArray().End(); iter++)
      {
        actual_joint_torque_.push_back(iter->GetDouble());
      }
      target_joint_pose_.clear();
      for(auto iter = obj["target_joint_pose"].GetArray().Begin(); iter != obj["target_joint_pose"].GetArray().End(); iter++)
      {
        target_joint_pose_.push_back(iter->GetDouble());
      }
      target_joint_speed_.clear();
      for(auto iter = obj["target_joint_speed"].GetArray().Begin(); iter != obj["target_joint_speed"].GetArray().End(); iter++)
      {
        target_joint_speed_.push_back(iter->GetDouble());
      }
      target_joint_acc_.clear();
      for(auto iter = obj["target_joint_acc"].GetArray().Begin(); iter != obj["target_joint_acc"].GetArray().End(); iter++)
      {
        target_joint_acc_.push_back(iter->GetDouble());
      }
      target_joint_torque_.clear();
      for(auto iter = obj["target_joint_torque"].GetArray().Begin(); iter != obj["target_joint_torque"].GetArray().End(); iter++)
      {
    ...
    // Serialize
    bool KinData::Serialize(rapidjson::Writer<rapidjson::StringBuffer> *writer) const
    {
      writer->StartObject();
      writer->Key("actual_joint_pose");
      writer->StartArray();
      for(auto iter = actual_joint_pose_.begin(); iter != actual_joint_pose_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
      ...
      writer->StartArray();
      for(auto iter = actual_joint_speed_.begin(); iter != actual_joint_speed_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
      ...
      writer->StartArray();
      for(auto iter = actual_joint_acc_.begin(); iter != actual_joint_acc_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
      ...
      writer->StartArray();
      for(auto iter = actual_joint_torque_.begin(); iter != actual_joint_torque_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
      ...
      writer->StartArray();
      for(auto iter = target_joint_pose_.begin(); iter != target_joint_pose_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
      ...
      writer->StartArray();
      for(auto iter = target_joint_speed_.begin(); iter != target_joint_speed_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
      ...
      writer->StartArray();
      for(auto iter = target_joint_acc_.begin(); iter != target_joint_acc_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
      ...
      writer->StartArray();
      for(auto iter = target_joint_torque_.begin(); iter != target_joint_torque_.end(); iter++)
      {
        writer->Double(*iter);
      }
      writer->EndArray();
