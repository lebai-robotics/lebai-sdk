...
std::string JSONBase::ToJSONString() const {
  ...
  rapidjson::Writer<rapidjson::StringBuffer> writer(
      ss);  // Can also use writer for condensed formatting
  if (Serialize(&writer)) {
...
// std::string JSONBase::ToJSONRpcReqString(const std::string & method, int id)
// const
// {
// 	rapidjson::StringBuffer ss;
// 	rapidjson::Writer<rapidjson::StringBuffer> writer(ss);		// Can
// also use Writer for condensed formatting 	writer.StartObject();
...
// 		std::cout << "Error: " << error_id << " " << error_msg <<
